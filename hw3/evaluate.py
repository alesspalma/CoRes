...
def main(test_path: str, endpoint: str, batch_size=32):
    ...
    while True:
        ...
        try:
            response = requests.post(endpoint, json={"sentences": prediction_samples[:1]}).json()
...
def evaluate(predictions_s, samples):
    ...
    for pred, label in zip(predictions_s, samples):
        ...
        if gold_both_wrong:
            ...
            gold_entity_offset = label["offset_A"] if label["is_coref_A"] == "TRUE" else label["offset_B"]
            gold_entity = label["entity_A"] if label["is_coref_A"] == "TRUE" else label["entity_B"]
            if (
